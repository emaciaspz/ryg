...
/**
 * Función para capturar o cargar
 * la foto
 */
...
/**
 * Función para desplegar el menu de opciones
 * de carga de imagenes
 * @param {*} id 
 */
function openFotoOptions(id) {
    window.localStorage.removeItem("idElementoFoto");
    fn.showActionSheet();
    window.localStorage.setItem("idElementoFoto", id);
}
...
/**
 * Función que setea un JSON con los
...
function showFotosCargadas(lstFotos) {
    console.error(device.platform, lstFotos)
    ...
    for (let i = 0; i < lstFotos.length; i++) {
        ...
        html += `
            <tr>
                ...
                <td><img width="130px" data-source="${foto.up_documento}"
                        src="${device.platform == 'iOS' ? foto.base64 : foto.up_documento}" 
...
function downloadPhoto(fileEntry, fotos_id, callback) {
    ...
    ft.download(apiURL, fileURL, function (photoEntry) {
            console.error('photoEntry', photoEntry)
            photoEntry.file(function (file) {
                var reader = new FileReader();
                reader.onloadend = function () {
                    if (device.platform == 'iOS') {
                        callback({
                            up_documento: photoEntry.fullPath,
                            base64: this.result
                        });
                    } else {
                        callback({
                            up_documento: photoEntry.nativeURL,
                            base64: this.result
                        });
                    }
                }
